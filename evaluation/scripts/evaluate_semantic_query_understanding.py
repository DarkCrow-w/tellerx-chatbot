...
from app.core.container import application_container
...
def main() -> int:
    ...
    understanding_service = container.query_understanding
    ...
    with SessionLocal() as db:
        for case in cases:
            ...
            evidence = retrieval_service.search(
                case["question"], [args.project_id], query_plan=plan
            )
            corpus = "\n".join(
            ...
            passed = (
                bool(evidence) == bool(case["expect_evidence"])
                and not missing
                and plan.strategy == "semantic-qwen-v1"
            )
            result = {
                "id": case["id"],
                "passed": passed,
                ...
                "requested_facts": list(plan.requested_facts),
                "constraints": list(plan.constraints),
                "retrieval_queries": list(plan.retrieval_queries),
                "evidence_count": len(evidence),
                "evidence_files": list(dict.fromkeys(item.filename for item in evidence)),
    ...
    print(
        json.dumps(
            {key: report[key] for key in ("cases", "passed", "pass_rate", "latency_ms")},
            ensure_ascii=False,
        )
    )
