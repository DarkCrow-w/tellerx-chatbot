...
from app.services import candidate_ranking, candidate_selection
...
def main():
    ...
    for file in ["scenario-answers.jsonl", "scenario-flash-answers.jsonl"]:
        rows.update(
            {r["id"]: r for r in map(json.loads, (run.OUTPUT / file).read_text().splitlines())}
        )
    probes = [
        ("q015", "青禾清算", "签名算法 密钥轮换周期", "签名密钥轮换周期"),
        ("q132", "长风容量", "生产环境 测试环境 并发上限", "并发上限"),
        ("q005", "澄海支付", "签名算法 密钥轮换周期", "签名密钥轮换周期"),
        ("q006", "澄海支付", "同步处理超时 队列 积压 告警 值班角色", "DLQ-8801"),
    ]
    owners = {
        "_retrieve_for_statuses": (retriever, "_retrieve_for_statuses"),
        "_merge_fused": (candidate_ranking, "merge_fused"),
        "_deduplicate_content": (candidate_ranking, "deduplicate_content"),
        "_enforce_exact_identifiers": (candidate_selection, "enforce_exact_identifiers"),
        "_prefer_complete_entity_matches": (candidate_selection, "prefer_complete_entity_matches"),
        "_select_rerank_candidate_pool": (candidate_selection, "select_rerank_candidate_pool"),
    }
    originals = {name: getattr(owner, attr) for name, (owner, attr) in owners.items()}
    active = {}
    ...
    def details(results):
        ...
        for rank, r in enumerate(results, 1):
            ...
            if source.get("filename") in active["expected_files"] and active[
                "needle"
            ] in source.get("content", ""):
                hits.append(
                    {
                        "rank": rank,
                        "filename": source.get("filename"),
                        "heading": source.get("title_path"),
    ...
    for name, method in originals.items():
        ...
        setattr(owner, attr, wrapped(name, method))
    output = []
    for case_id, business, facts, needle in probes:
        ...
        plan = QueryPlan(
            **{
                f.name: plan_data[f.name]
                for f in dataclasses.fields(QueryPlan)
                if f.name in plan_data
            }
        )
        outcome = retriever.search_with_scope(record["question"], [], query_plan=plan)
        active["baseline_evidence"] = [
            {
                "filename": e.filename,
                "heading": e.heading_path,
                "topic_match": needle in e.content,
                "target": needle in e.content and e.filename in record["expected_files"],
            }
        ...
        active["stages"] = []
        filter_method = candidate_selection.prefer_complete_entity_matches
        candidate_selection.prefer_complete_entity_matches = (
            lambda query, rows, linked_identifiers=None, **kwargs: rows
        )
        ablation = retriever.search_with_scope(record["question"], [], query_plan=plan)
        candidate_selection.prefer_complete_entity_matches = filter_method
        active["filter_ablation_stages"] = active["stages"]
        active["stages"] = baseline_stages
        active["filter_ablation_evidence"] = [
            {
                "filename": e.filename,
                "heading": e.heading_path,
                "topic_match": needle in e.content,
        ...
        fact_hits = (
            originals["_retrieve_for_statuses"](facts, [], ["approved"], document_ids=doc_ids)
            if doc_ids
            else []
        )
        active["fact_only_in_business_documents"] = details(fact_hits)
        active["prototype_note"] = (
            "人工拆分问题；业务文档由名称检索发现；只验证候选召回，不代表完整问答或自动方案通过率。"
        )
        output.append(active)
        print(
            case_id,
            "baseline",
            sum(x["target"] for x in active["baseline_evidence"]),
            "scoped fact hits",
            active["fact_only_in_business_documents"]["target_hits"],
            flush=True,
        )
    for name, method in originals.items():
        owner, attr = owners[name]
        setattr(owner, attr, method)
    (run.OUTPUT / "global-scope-probe.json").write_text(
