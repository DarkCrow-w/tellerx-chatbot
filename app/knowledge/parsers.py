...
class DocumentParser:
    ...
    def parse(self, path: Path) -> tuple[list[ParsedUnit], list[str]]:
        ...
        if suffix not in self.allowed_suffixes:
            if suffix == ".one":
                raise UnsupportedDocument("Native OneNote .one files must be exported to DOCX, PDF, or HTML")
    ...
    def _parse_pdf_native(path: Path) -> list[ParsedUnit]:
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            # PDF extraction can insert a newline inside an ASCII token at a
            # visual wrap boundary (for example ``TX-0929-R\nEADY``).
            text = re.sub(
            ...
            if text.strip():
                units.append(ParsedUnit(text=text, page_number=page_number, heading_path=f"Page {page_number}"))
        return units
