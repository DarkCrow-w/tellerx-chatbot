...
正常请求在 INFO 级别只记录一条完成日志；请求开始、SDK 调用细节、文档/版本关联 ID
及模型用量落库确认放在 DEBUG 级别。入库使用 `job_id` 关联，索引发布使用 `event_id`
关联；数据主键和持久化问答追踪 ID 保留不变。

切块日志记录解析单元数、分块数、估算 Token 数及耗时；Embedding 日志记录输入去重、
缓存命中、待生成数量、批次进度及耗时，不逐块打印正文、向量或哈希。索引日志记录
预期/实际分块数，失败时明确记录是否进入重试。API 密钥、问题正文和 Prompt 不作为
阶段日志字段，`tokens` 字段表示用量统计。

`ALLOW_BM25_ONLY=true` 时，模型服务异常仍可降级为关键词检索，并记录状态码和错误码；
程序错误、数据库异常和存储失败会继续抛出，不再被宽泛捕获后当作正常降级。
向量响应的数量、索引、维度及数值有效性会校验，错误响应作为模型服务异常处理。
入库失败会保存失败阶段并记录堆栈；索引事件保留既有重试机制。
