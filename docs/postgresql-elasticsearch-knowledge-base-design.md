...
2. **确定性查询规范化**：Unicode NFKC、全半角、大小写、空白；抽取业务编号、错误码、接口名、缩写、文件名和表格坐标。
3. **BM25 召回**：正文、标题、文件名和精确字段加权，取前 80；精确编号命中获得强加权。
4. **向量召回**：用与索引相同 fingerprint 的 Qwen Embedding，过滤后 kNN 取前 80，`num_candidates` 初始 400 并由压测调优。
5. **RRF 融合**：应用侧按候选 rank 融合，初始 `rank_constant=60`、窗口 100；不得直接混加不可比较的 BM25 和余弦分数。
6. **规则重排**：当前 `approved`、标题匹配、精确实体完整命中加权；废弃版本排除，草稿只在用户显式要求或已批准证据不足时补充并标识。
7. **去重与限额**：相同 `content_hash` 去重；限制单文档/单章节占比，防止一个长文档淹没其他证据。
8. **Qwen Rerank**：前 30 个候选重排，保留前 8；API 不可用时可回退到 RRF，但正式验收必须单列质量差异。
9. **邻接补充**：仅为入选 Chunk 加入父块或相邻块，不对所有候选扩张；上下文普通上限 4,000 tokens、复杂问题上限 7,000。
10. **证据充分性判断**：依据离线标定的阈值、关键实体覆盖、条件/时间覆盖和来源一致性，不使用“总会返回 k 条”的近邻结果冒充证据。
